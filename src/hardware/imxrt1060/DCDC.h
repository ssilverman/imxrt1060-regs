...
// Structure type to access the DCDC Converter (DCDC) registers.
//
...
constexpr DCDC_Reg<&DCDC_Layout::REG1, 2, 12> LP_CMP_ISRC_SEL;      // Low Power Comparator Current Bias
    ...
    // 0b10..200 nA
    // 0b11..400 nA
constexpr DCDC_Reg<&DCDC_Layout::REG1, 1,  9> REG_RLOAD_SW;         // This controls the load resistor of the internal regulator of DCDC
    // 0b0..Load resistor disconnected
    // 0b1..Load resistor connected
constexpr DCDC_Reg<&DCDC_Layout::REG1, 2,  7> REG_FBK_SEL;          // Select the feedback point of the internal regulator
...
constexpr DCDC_Reg<&DCDC_Layout::REG2, 1, 13> LOOPCTRL_HYST_SIGN;      // Invert the sign of the hysteresis in DC-DC analog comparators.
    // 0b0..Do not invert sign of the hysteresis
    // 0b1..Invert sign of the hysteresis
constexpr DCDC_Reg<&DCDC_Layout::REG2, 1, 12> LOOPCTRL_RCSCALE_THRSH;  // Increase the threshold detection for RC scale circuit.
...
constexpr DCDC_Reg<&DCDC_Layout::REG2, 3,  6> LOOPCTRL_DC_FF;          // Two's complement feed forward step in duty cycle in the switching DC-DC converter
}  // namespace REG2
...
constexpr DCDC_Reg<&DCDC_Layout::REG3, 1, 24> MINPWR_DC_HALFCLK;  // Set DCDC clock to half frequency for continuous mode
