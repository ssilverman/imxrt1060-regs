...
constexpr FLEXSPI_Reg<&FLEXSPI_Layout::MCR0, 1, 14> SCKFREERUNEN;   // This bit is used to force SCLK output free-running.
    ...
    // 0b0..Disable.
    // 0b1..Enable.
constexpr FLEXSPI_Reg<&FLEXSPI_Layout::MCR0, 1, 13> COMBINATIONEN;  // This bit is to support Flash Octal mode access by combining Port A and B Data pins (A_DATA[3:0] and B_DATA[3:0]), when Port A and Port B are of 4 bit data width.
    // 0b0..Disable.
    // 0b1..Enable.
...
namespace MCR2 {
constexpr FLEXSPI_Reg<&FLEXSPI_Layout::MCR2, 8, 24> RESUMEWAIT;    // Wait cycle (in AHB clock cycle) for idle state before suspended command sequence resumed.
constexpr FLEXSPI_Reg<&FLEXSPI_Layout::MCR2, 1, 19> SCKBDIFFOPT;   // B_SCLK pad can be used as A_SCLK differential clock output (inverted clock to A_SCLK).
    // In this case, port B
...
constexpr FLEXSPI_Reg<&FLEXSPI_Layout::AHBCR, 1,  3> CACHABLEEN;    // Enable AHB bus cachable read access support.
    ...
    //     first.
constexpr FLEXSPI_Reg<&FLEXSPI_Layout::AHBCR, 1,  2> CLRAHBTXBUF;   // Clear the status/pointers of AHB TX Buffer.
    // Auto-cleared.
constexpr FLEXSPI_Reg<&FLEXSPI_Layout::AHBCR, 1,  1> CLRAHBRXBUF;   // Clear the status/pointers of AHB RX Buffer.
    // Auto-cleared.
constexpr FLEXSPI_Reg<&FLEXSPI_Layout::AHBCR, 1,  0> APAREN;        // Parallel mode enabled for AHB triggered Command (both read and write) .
...
constexpr FLEXSPI_Reg<&FLEXSPI_Layout::INTEN, 1, 10> AHBBUSTIMEOUTEN;  // AHB Bus timeout interrupt.
    // Refer Interrupts chapter for more details.
constexpr FLEXSPI_Reg<&FLEXSPI_Layout::INTEN, 1,  9> SCKSTOPBYWREN;    // SCLK is stopped during command sequence because Async TX FIFO empty interrupt enable.
constexpr FLEXSPI_Reg<&FLEXSPI_Layout::INTEN, 1,  8> SCKSTOPBYRDEN;    // SCLK is stopped during command sequence because Async RX FIFO full interrupt enable.
...
constexpr FLEXSPI_Reg<&FLEXSPI_Layout::INTR, 1, 10, 0x0> AHBBUSTIMEOUT;  // AHB Bus timeout interrupt.
    // Refer Interrupts chapter for more details.
constexpr FLEXSPI_Reg<&FLEXSPI_Layout::INTR, 1,  9, 0x0> SCKSTOPBYWR;    // SCLK is stopped during command sequence because Async TX FIFO empty interrupt.
constexpr FLEXSPI_Reg<&FLEXSPI_Layout::INTR, 1,  8, 0x0> SCKSTOPBYRD;    // SCLK is stopped during command sequence because Async RX FIFO full interrupt.
...
constexpr AHBRXBUFCR0_Reg<Index, 1, 31> PREFETCHEN;  // AHB Read Prefetch Enable for current AHB RX Buffer corresponding Master.
template <size_t Index>
...
template <size_t Index>
constexpr DLLCR_Reg<Index, 4,  3> SLVDLYTARGET;  // The delay target for slave delay line is: ((SLVDLYTARGET+1) * 1/32 * clock cycle of reference clock (serial root clock).
    // If serial root clock is >= 100 MHz, DLLEN set to 0x1, OVRDEN set to =0x0, then SLVDLYTARGET setting of 0xF is recommended.
template <size_t Index>
...
// Status Register 0
namespace STS0 {
constexpr FLEXSPI_Reg<&FLEXSPI_Layout::STS0, 2, 2> ARBCMDSRC;  // This status field indicates the trigger source of current command sequence granted by arbitrator.
    // This field
...
constexpr FLEXSPI_Reg<&FLEXSPI_Layout::STS0, 1, 1> ARBIDLE;    // This status bit indicates the state machine in ARB_CTL is busy and there is command sequence granted by arbitrator and not finished yet on FlexSPI interface.
    //     When ARB_CTL state (ARBIDLE=0x1) is idle, there
...
constexpr FLEXSPI_Reg<&FLEXSPI_Layout::STS0, 1, 0> SEQIDLE;    // This status bit indicates the state machine in SEQ_CTL is idle and there is command sequence executing on FlexSPI interface.
}  // namespace STS0
