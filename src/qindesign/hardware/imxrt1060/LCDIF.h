...
constexpr LCDIF_Reg<&LCDIF_Layout::CTRL, 5, 21> SHIFT_NUM_BITS;        // The data to be transmitted is shifted left or right by this number of bits.
constexpr LCDIF_Reg<&LCDIF_Layout::CTRL, 1, 19> BYPASS_COUNT;          // When this bit is 0, it means that LCDIF will stop the block operation and turn off the RUN bit after the amount of data indicated by the LCDIF_TRANSFER_COUNT register has been transferred out
constexpr LCDIF_Reg<&LCDIF_Layout::CTRL, 1, 17> DOTCLK_MODE;           // Set this bit to 1 to make the hardware go into the DOTCLK mode, i.e. VSYNC/HSYNC/DOTCLK/ENABLE interface mode.
    //     ENABLE is optional, selected by the ENABLE_PRESENT bit. Toggle this bit from 1 to 0 to make the
...
constexpr LCDIF_Reg<&LCDIF_Layout::CTRL, 2, 12> CSC_DATA_SWIZZLE;      // This field specifies how to swap the bytes after the data has been converted into an internal representation of 24 bits per pixel and before it is transmitted over the LCD interface bus
    // 0b00..Little Endian byte ordering (same as NO_SWAP).
    // 0b00..No byte swapping.(Little endian)
    // 0b01..Big Endian swap (swap bytes 0,3 and 1,2).
    // 0b01..Swizzle all bytes, swap bytes 0,3 and 1,2 (aka Big Endian).
...
constexpr LCDIF_Reg<&LCDIF_Layout::CTRL, 1,  6> ENABLE_PXP_HANDSHAKE;  // If this bit is set and LCDIF_MASTER bit is set, the LCDIF will act as bus master and the handshake mechanism between LCDIF and PXP will be turned on
constexpr LCDIF_Reg<&LCDIF_Layout::CTRL, 1,  5> MASTER;                // Set this bit to make the LCDIF act as a bus master
constexpr LCDIF_Reg<&LCDIF_Layout::CTRL, 1,  3> DATA_FORMAT_16_BIT;    // When this bit is 1 and WORD_LENGTH = 0, it implies that the 16-bit data is in ARGB555 format
constexpr LCDIF_Reg<&LCDIF_Layout::CTRL, 1,  2> DATA_FORMAT_18_BIT;    // Used only when WORD_LENGTH = 2, i.e. 18-bit.
    ...
    //     do not contain any useful data.
constexpr LCDIF_Reg<&LCDIF_Layout::CTRL, 1,  1> DATA_FORMAT_24_BIT;    // Used only when WORD_LENGTH = 3, i.e. 24-bit.
    // Note that this applies to both packed and unpacked 24-bit data.
...
constexpr LCDIF_Reg<&LCDIF_Layout::CTRL, 1,  0> RUN;                   // When this bit is set by software, the LCDIF will begin transferring data between the SoC and the display
}  // namespace CTRL
...
constexpr LCDIF_Reg<&LCDIF_Layout::CTRL_SET, 1, 26, 0x0> DATA_SHIFT_DIR;        // Use this bit to determine the direction of shift of transmit data.
    // 0b0..Data to be transmitted is shifted LEFT by SHIFT_NUM_BITS bits.
    // 0b1..Data to be transmitted is shifted RIGHT by SHIFT_NUM_BITS bits.
constexpr LCDIF_Reg<&LCDIF_Layout::CTRL_SET, 5, 21, 0x0> SHIFT_NUM_BITS;        // The data to be transmitted is shifted left or right by this number of bits.
constexpr LCDIF_Reg<&LCDIF_Layout::CTRL_SET, 1, 19, 0x0> BYPASS_COUNT;          // When this bit is 0, it means that LCDIF will stop the block operation and turn off the RUN bit after the amount of data indicated by the LCDIF_TRANSFER_COUNT register has been transferred out
constexpr LCDIF_Reg<&LCDIF_Layout::CTRL_SET, 1, 17, 0x0> DOTCLK_MODE;           // Set this bit to 1 to make the hardware go into the DOTCLK mode, i.e. VSYNC/HSYNC/DOTCLK/ENABLE interface mode.
    //     ENABLE is optional, selected by the ENABLE_PRESENT bit. Toggle this bit from 1 to 0 to make the
    //     hardware go out of DOTCLK mode after completing all data transfer and deasserting the RUN bit.
constexpr LCDIF_Reg<&LCDIF_Layout::CTRL_SET, 2, 14, 0x0> INPUT_DATA_SWIZZLE;    // This field specifies how to swap the bytes fetched by the bus master interface
    ...
    // 0b10..Swap half-words.
    // 0b11..Swap bytes within each half-word.
constexpr LCDIF_Reg<&LCDIF_Layout::CTRL_SET, 2, 12, 0x0> CSC_DATA_SWIZZLE;      // This field specifies how to swap the bytes after the data has been converted into an internal representation of 24 bits per pixel and before it is transmitted over the LCD interface bus
    // 0b00..Little Endian byte ordering (same as NO_SWAP).
    // 0b00..No byte swapping.(Little endian)
    // 0b01..Big Endian swap (swap bytes 0,3 and 1,2).
    // 0b01..Swizzle all bytes, swap bytes 0,3 and 1,2 (aka Big Endian).
...
constexpr LCDIF_Reg<&LCDIF_Layout::CTRL_SET, 2,  8, 0x0> WORD_LENGTH;           // Input data format.
    ...
    // 0b10..Input data is 18 bits per pixel.
    // 0b11..Input data is 24 bits per pixel.
constexpr LCDIF_Reg<&LCDIF_Layout::CTRL_SET, 1,  6, 0x0> ENABLE_PXP_HANDSHAKE;  // If this bit is set and LCDIF_MASTER bit is set, the LCDIF will act as bus master and the handshake mechanism between LCDIF and PXP will be turned on
constexpr LCDIF_Reg<&LCDIF_Layout::CTRL_SET, 1,  5, 0x0> MASTER;                // Set this bit to make the LCDIF act as a bus master
constexpr LCDIF_Reg<&LCDIF_Layout::CTRL_SET, 1,  3, 0x0> DATA_FORMAT_16_BIT;    // When this bit is 1 and WORD_LENGTH = 0, it implies that the 16-bit data is in ARGB555 format
constexpr LCDIF_Reg<&LCDIF_Layout::CTRL_SET, 1,  2, 0x0> DATA_FORMAT_18_BIT;    // Used only when WORD_LENGTH = 2, i.e. 18-bit.
    // 0b0..Data input to the block is in 18 bpp format, such that lower 18 bits contain RGB 666 and upper 14 bits
    //     do not contain any useful data.
    // 0b1..Data input to the block is in 18 bpp format, such that upper 18 bits contain RGB 666 and lower 14 bits
    //     do not contain any useful data.
constexpr LCDIF_Reg<&LCDIF_Layout::CTRL_SET, 1,  1, 0x0> DATA_FORMAT_24_BIT;    // Used only when WORD_LENGTH = 3, i.e. 24-bit.
    // Note that this applies to both packed and unpacked 24-bit data.
    // 0b0..Data input to the block is in 24 bpp format, such that all RGB 888 data is contained in 24 bits.
    // 0b1..Data input to the block is actually RGB 18 bpp, but there is 1 color per byte, hence the upper 2 bits in
    //     each byte do not contain any useful data, and should be dropped.
constexpr LCDIF_Reg<&LCDIF_Layout::CTRL_SET, 1,  0, 0x0> RUN;                   // When this bit is set by software, the LCDIF will begin transferring data between the SoC and the display
}  // namespace CTRL_SET

// LCDIF General Control Register
namespace CTRL_CLR {
constexpr LCDIF_Reg<&LCDIF_Layout::CTRL_CLR, 1, 31, 0x0> SFTRST;                // This bit must be set to zero to enable normal operation of the LCDIF
constexpr LCDIF_Reg<&LCDIF_Layout::CTRL_CLR, 1, 30, 0x0> CLKGATE;               // This bit must be set to zero for normal operation
constexpr LCDIF_Reg<&LCDIF_Layout::CTRL_CLR, 1, 26, 0x0> DATA_SHIFT_DIR;        // Use this bit to determine the direction of shift of transmit data.
    // 0b0..Data to be transmitted is shifted LEFT by SHIFT_NUM_BITS bits.
    // 0b1..Data to be transmitted is shifted RIGHT by SHIFT_NUM_BITS bits.
constexpr LCDIF_Reg<&LCDIF_Layout::CTRL_CLR, 5, 21, 0x0> SHIFT_NUM_BITS;        // The data to be transmitted is shifted left or right by this number of bits.
constexpr LCDIF_Reg<&LCDIF_Layout::CTRL_CLR, 1, 19, 0x0> BYPASS_COUNT;          // When this bit is 0, it means that LCDIF will stop the block operation and turn off the RUN bit after the amount of data indicated by the LCDIF_TRANSFER_COUNT register has been transferred out
constexpr LCDIF_Reg<&LCDIF_Layout::CTRL_CLR, 1, 17, 0x0> DOTCLK_MODE;           // Set this bit to 1 to make the hardware go into the DOTCLK mode, i.e. VSYNC/HSYNC/DOTCLK/ENABLE interface mode.
    //     ENABLE is optional, selected by the ENABLE_PRESENT bit. Toggle this bit from 1 to 0 to make the
    //     hardware go out of DOTCLK mode after completing all data transfer and deasserting the RUN bit.
constexpr LCDIF_Reg<&LCDIF_Layout::CTRL_CLR, 2, 14, 0x0> INPUT_DATA_SWIZZLE;    // This field specifies how to swap the bytes fetched by the bus master interface
    // 0b00..Little Endian byte ordering (same as NO_SWAP).
    // 0b00..No byte swapping.(Little endian)
    // 0b01..Big Endian swap (swap bytes 0,3 and 1,2).
    // 0b01..Swizzle all bytes, swap bytes 0,3 and 1,2 (aka Big Endian).
    // 0b10..Swap half-words.
    // 0b11..Swap bytes within each half-word.
constexpr LCDIF_Reg<&LCDIF_Layout::CTRL_CLR, 2, 12, 0x0> CSC_DATA_SWIZZLE;      // This field specifies how to swap the bytes after the data has been converted into an internal representation of 24 bits per pixel and before it is transmitted over the LCD interface bus
    // 0b00..Little Endian byte ordering (same as NO_SWAP).
    // 0b00..No byte swapping.(Little endian)
    // 0b01..Big Endian swap (swap bytes 0,3 and 1,2).
    // 0b01..Swizzle all bytes, swap bytes 0,3 and 1,2 (aka Big Endian).
    // 0b10..Swap half-words.
    // 0b11..Swap bytes within each half-word.
constexpr LCDIF_Reg<&LCDIF_Layout::CTRL_CLR, 2, 10, 0x0> LCD_DATABUS_WIDTH;     // LCD Data bus transfer width.
    // When LUT enabled, this field should be set to 0x01.
    // 0b00..16-bit data bus mode.
    // 0b01..8-bit data bus mode.
    // 0b10..18-bit data bus mode.
...
constexpr LCDIF_Reg<&LCDIF_Layout::CTRL_CLR, 2,  8, 0x0> WORD_LENGTH;           // Input data format.
    ...
    // 0b10..Input data is 18 bits per pixel.
    // 0b11..Input data is 24 bits per pixel.
constexpr LCDIF_Reg<&LCDIF_Layout::CTRL_CLR, 1,  6, 0x0> ENABLE_PXP_HANDSHAKE;  // If this bit is set and LCDIF_MASTER bit is set, the LCDIF will act as bus master and the handshake mechanism between LCDIF and PXP will be turned on
constexpr LCDIF_Reg<&LCDIF_Layout::CTRL_CLR, 1,  5, 0x0> MASTER;                // Set this bit to make the LCDIF act as a bus master
constexpr LCDIF_Reg<&LCDIF_Layout::CTRL_CLR, 1,  3, 0x0> DATA_FORMAT_16_BIT;    // When this bit is 1 and WORD_LENGTH = 0, it implies that the 16-bit data is in ARGB555 format
constexpr LCDIF_Reg<&LCDIF_Layout::CTRL_CLR, 1,  2, 0x0> DATA_FORMAT_18_BIT;    // Used only when WORD_LENGTH = 2, i.e. 18-bit.
    // 0b0..Data input to the block is in 18 bpp format, such that lower 18 bits contain RGB 666 and upper 14 bits
    //     do not contain any useful data.
    // 0b1..Data input to the block is in 18 bpp format, such that upper 18 bits contain RGB 666 and lower 14 bits
    //     do not contain any useful data.
constexpr LCDIF_Reg<&LCDIF_Layout::CTRL_CLR, 1,  1, 0x0> DATA_FORMAT_24_BIT;    // Used only when WORD_LENGTH = 3, i.e. 24-bit.
    // Note that this applies to both packed and unpacked 24-bit data.
    // 0b0..Data input to the block is in 24 bpp format, such that all RGB 888 data is contained in 24 bits.
    // 0b1..Data input to the block is actually RGB 18 bpp, but there is 1 color per byte, hence the upper 2 bits in
    //     each byte do not contain any useful data, and should be dropped.
constexpr LCDIF_Reg<&LCDIF_Layout::CTRL_CLR, 1,  0, 0x0> RUN;                   // When this bit is set by software, the LCDIF will begin transferring data between the SoC and the display
}  // namespace CTRL_CLR

// LCDIF General Control Register
namespace CTRL_TOG {
constexpr LCDIF_Reg<&LCDIF_Layout::CTRL_TOG, 1, 31, 0x0> SFTRST;                // This bit must be set to zero to enable normal operation of the LCDIF
constexpr LCDIF_Reg<&LCDIF_Layout::CTRL_TOG, 1, 30, 0x0> CLKGATE;               // This bit must be set to zero for normal operation
constexpr LCDIF_Reg<&LCDIF_Layout::CTRL_TOG, 1, 26, 0x0> DATA_SHIFT_DIR;        // Use this bit to determine the direction of shift of transmit data.
    // 0b0..Data to be transmitted is shifted LEFT by SHIFT_NUM_BITS bits.
    // 0b1..Data to be transmitted is shifted RIGHT by SHIFT_NUM_BITS bits.
constexpr LCDIF_Reg<&LCDIF_Layout::CTRL_TOG, 5, 21, 0x0> SHIFT_NUM_BITS;        // The data to be transmitted is shifted left or right by this number of bits.
constexpr LCDIF_Reg<&LCDIF_Layout::CTRL_TOG, 1, 19, 0x0> BYPASS_COUNT;          // When this bit is 0, it means that LCDIF will stop the block operation and turn off the RUN bit after the amount of data indicated by the LCDIF_TRANSFER_COUNT register has been transferred out
constexpr LCDIF_Reg<&LCDIF_Layout::CTRL_TOG, 1, 17, 0x0> DOTCLK_MODE;           // Set this bit to 1 to make the hardware go into the DOTCLK mode, i.e. VSYNC/HSYNC/DOTCLK/ENABLE interface mode.
    //     ENABLE is optional, selected by the ENABLE_PRESENT bit. Toggle this bit from 1 to 0 to make the
    //     hardware go out of DOTCLK mode after completing all data transfer and deasserting the RUN bit.
constexpr LCDIF_Reg<&LCDIF_Layout::CTRL_TOG, 2, 14, 0x0> INPUT_DATA_SWIZZLE;    // This field specifies how to swap the bytes fetched by the bus master interface
    // 0b00..Little Endian byte ordering (same as NO_SWAP).
    // 0b00..No byte swapping.(Little endian)
    // 0b01..Big Endian swap (swap bytes 0,3 and 1,2).
    // 0b01..Swizzle all bytes, swap bytes 0,3 and 1,2 (aka Big Endian).
    // 0b10..Swap half-words.
    // 0b11..Swap bytes within each half-word.
constexpr LCDIF_Reg<&LCDIF_Layout::CTRL_TOG, 2, 12, 0x0> CSC_DATA_SWIZZLE;      // This field specifies how to swap the bytes after the data has been converted into an internal representation of 24 bits per pixel and before it is transmitted over the LCD interface bus
    // 0b00..Little Endian byte ordering (same as NO_SWAP).
    // 0b00..No byte swapping.(Little endian)
    // 0b01..Big Endian swap (swap bytes 0,3 and 1,2).
    // 0b01..Swizzle all bytes, swap bytes 0,3 and 1,2 (aka Big Endian).
    // 0b10..Swap half-words.
    // 0b11..Swap bytes within each half-word.
constexpr LCDIF_Reg<&LCDIF_Layout::CTRL_TOG, 2, 10, 0x0> LCD_DATABUS_WIDTH;     // LCD Data bus transfer width.
    // When LUT enabled, this field should be set to 0x01.
    // 0b00..16-bit data bus mode.
    // 0b01..8-bit data bus mode.
    // 0b10..18-bit data bus mode.
...
constexpr LCDIF_Reg<&LCDIF_Layout::CTRL_TOG, 2,  8, 0x0> WORD_LENGTH;           // Input data format.
    ...
    // 0b10..Input data is 18 bits per pixel.
    // 0b11..Input data is 24 bits per pixel.
constexpr LCDIF_Reg<&LCDIF_Layout::CTRL_TOG, 1,  6, 0x0> ENABLE_PXP_HANDSHAKE;  // If this bit is set and LCDIF_MASTER bit is set, the LCDIF will act as bus master and the handshake mechanism between LCDIF and PXP will be turned on
constexpr LCDIF_Reg<&LCDIF_Layout::CTRL_TOG, 1,  5, 0x0> MASTER;                // Set this bit to make the LCDIF act as a bus master
constexpr LCDIF_Reg<&LCDIF_Layout::CTRL_TOG, 1,  3, 0x0> DATA_FORMAT_16_BIT;    // When this bit is 1 and WORD_LENGTH = 0, it implies that the 16-bit data is in ARGB555 format
constexpr LCDIF_Reg<&LCDIF_Layout::CTRL_TOG, 1,  2, 0x0> DATA_FORMAT_18_BIT;    // Used only when WORD_LENGTH = 2, i.e. 18-bit.
    // 0b0..Data input to the block is in 18 bpp format, such that lower 18 bits contain RGB 666 and upper 14 bits
    //     do not contain any useful data.
    // 0b1..Data input to the block is in 18 bpp format, such that upper 18 bits contain RGB 666 and lower 14 bits
    //     do not contain any useful data.
constexpr LCDIF_Reg<&LCDIF_Layout::CTRL_TOG, 1,  1, 0x0> DATA_FORMAT_24_BIT;    // Used only when WORD_LENGTH = 3, i.e. 24-bit.
    // Note that this applies to both packed and unpacked 24-bit data.
    // 0b0..Data input to the block is in 24 bpp format, such that all RGB 888 data is contained in 24 bits.
    // 0b1..Data input to the block is actually RGB 18 bpp, but there is 1 color per byte, hence the upper 2 bits in
    //     each byte do not contain any useful data, and should be dropped.
constexpr LCDIF_Reg<&LCDIF_Layout::CTRL_TOG, 1,  0, 0x0> RUN;                   // When this bit is set by software, the LCDIF will begin transferring data between the SoC and the display
}  // namespace CTRL_TOG
...
constexpr LCDIF_Reg<&LCDIF_Layout::CTRL1, 1, 24> RECOVER_ON_UNDERFLOW;                // Set this bit to enable the LCDIF block to recover in the next field/frame if there was an underflow in the current field/frame
constexpr LCDIF_Reg<&LCDIF_Layout::CTRL1, 1, 23> INTERLACE_FIELDS;                    // Set this bit if it is required that the LCDIF block fetches odd lines in one field and even lines in the other field
constexpr LCDIF_Reg<&LCDIF_Layout::CTRL1, 1, 22> START_INTERLACE_FROM_SECOND_FIELD;   // The default is to grab the odd lines first and then the even lines
...
constexpr LCDIF_Reg<&LCDIF_Layout::CTRL1, 1, 20> IRQ_ON_ALTERNATE_FIELDS;             // If this bit is set, the LCDIF block will assert the cur_frame_done interrupt only on alternate fields, otherwise it will issue the interrupt on both odd and even field
...
constexpr LCDIF_Reg<&LCDIF_Layout::CTRL1, 1, 13> CUR_FRAME_DONE_IRQ_EN;               // This bit is set to 1 enable an interrupt every time the hardware enters in the vertical blanking state
constexpr LCDIF_Reg<&LCDIF_Layout::CTRL1, 1, 12> VSYNC_EDGE_IRQ_EN;                   // This bit is set to enable an interrupt every time the hardware encounters the leading VSYNC edge in the VSYNC and DOTCLK modes, or the beginning of every field in DVI mode
...
constexpr LCDIF_Reg<&LCDIF_Layout::CTRL1_SET, 1, 25, 0x0> BM_ERROR_IRQ;                       // This bit is set to indicate that an interrupt is requested by the LCDIF block
    // 0b0..No Interrupt Request Pending.
    // 0b1..Interrupt Request Pending.
constexpr LCDIF_Reg<&LCDIF_Layout::CTRL1_SET, 1, 24, 0x0> RECOVER_ON_UNDERFLOW;               // Set this bit to enable the LCDIF block to recover in the next field/frame if there was an underflow in the current field/frame
constexpr LCDIF_Reg<&LCDIF_Layout::CTRL1_SET, 1, 23, 0x0> INTERLACE_FIELDS;                   // Set this bit if it is required that the LCDIF block fetches odd lines in one field and even lines in the other field
constexpr LCDIF_Reg<&LCDIF_Layout::CTRL1_SET, 1, 22, 0x0> START_INTERLACE_FROM_SECOND_FIELD;  // The default is to grab the odd lines first and then the even lines
...
constexpr LCDIF_Reg<&LCDIF_Layout::CTRL1_SET, 1, 20, 0x0> IRQ_ON_ALTERNATE_FIELDS;            // If this bit is set, the LCDIF block will assert the cur_frame_done interrupt only on alternate fields, otherwise it will issue the interrupt on both odd and even field
...
constexpr LCDIF_Reg<&LCDIF_Layout::CTRL1_SET, 1, 13, 0x0> CUR_FRAME_DONE_IRQ_EN;              // This bit is set to 1 enable an interrupt every time the hardware enters in the vertical blanking state
constexpr LCDIF_Reg<&LCDIF_Layout::CTRL1_SET, 1, 12, 0x0> VSYNC_EDGE_IRQ_EN;                  // This bit is set to enable an interrupt every time the hardware encounters the leading VSYNC edge in the VSYNC and DOTCLK modes, or the beginning of every field in DVI mode
...
constexpr LCDIF_Reg<&LCDIF_Layout::CTRL1_CLR, 1, 25, 0x0> BM_ERROR_IRQ;                       // This bit is set to indicate that an interrupt is requested by the LCDIF block
    // 0b0..No Interrupt Request Pending.
    // 0b1..Interrupt Request Pending.
constexpr LCDIF_Reg<&LCDIF_Layout::CTRL1_CLR, 1, 24, 0x0> RECOVER_ON_UNDERFLOW;               // Set this bit to enable the LCDIF block to recover in the next field/frame if there was an underflow in the current field/frame
constexpr LCDIF_Reg<&LCDIF_Layout::CTRL1_CLR, 1, 23, 0x0> INTERLACE_FIELDS;                   // Set this bit if it is required that the LCDIF block fetches odd lines in one field and even lines in the other field
constexpr LCDIF_Reg<&LCDIF_Layout::CTRL1_CLR, 1, 22, 0x0> START_INTERLACE_FROM_SECOND_FIELD;  // The default is to grab the odd lines first and then the even lines
...
constexpr LCDIF_Reg<&LCDIF_Layout::CTRL1_CLR, 1, 20, 0x0> IRQ_ON_ALTERNATE_FIELDS;            // If this bit is set, the LCDIF block will assert the cur_frame_done interrupt only on alternate fields, otherwise it will issue the interrupt on both odd and even field
...
constexpr LCDIF_Reg<&LCDIF_Layout::CTRL1_CLR, 1, 13, 0x0> CUR_FRAME_DONE_IRQ_EN;              // This bit is set to 1 enable an interrupt every time the hardware enters in the vertical blanking state
constexpr LCDIF_Reg<&LCDIF_Layout::CTRL1_CLR, 1, 12, 0x0> VSYNC_EDGE_IRQ_EN;                  // This bit is set to enable an interrupt every time the hardware encounters the leading VSYNC edge in the VSYNC and DOTCLK modes, or the beginning of every field in DVI mode
...
constexpr LCDIF_Reg<&LCDIF_Layout::CTRL1_TOG, 1, 25, 0x0> BM_ERROR_IRQ;                       // This bit is set to indicate that an interrupt is requested by the LCDIF block
    // 0b0..No Interrupt Request Pending.
    // 0b1..Interrupt Request Pending.
constexpr LCDIF_Reg<&LCDIF_Layout::CTRL1_TOG, 1, 24, 0x0> RECOVER_ON_UNDERFLOW;               // Set this bit to enable the LCDIF block to recover in the next field/frame if there was an underflow in the current field/frame
constexpr LCDIF_Reg<&LCDIF_Layout::CTRL1_TOG, 1, 23, 0x0> INTERLACE_FIELDS;                   // Set this bit if it is required that the LCDIF block fetches odd lines in one field and even lines in the other field
constexpr LCDIF_Reg<&LCDIF_Layout::CTRL1_TOG, 1, 22, 0x0> START_INTERLACE_FROM_SECOND_FIELD;  // The default is to grab the odd lines first and then the even lines
...
constexpr LCDIF_Reg<&LCDIF_Layout::CTRL1_TOG, 1, 20, 0x0> IRQ_ON_ALTERNATE_FIELDS;            // If this bit is set, the LCDIF block will assert the cur_frame_done interrupt only on alternate fields, otherwise it will issue the interrupt on both odd and even field
...
constexpr LCDIF_Reg<&LCDIF_Layout::CTRL1_TOG, 1, 13, 0x0> CUR_FRAME_DONE_IRQ_EN;              // This bit is set to 1 enable an interrupt every time the hardware enters in the vertical blanking state
constexpr LCDIF_Reg<&LCDIF_Layout::CTRL1_TOG, 1, 12, 0x0> VSYNC_EDGE_IRQ_EN;                  // This bit is set to enable an interrupt every time the hardware encounters the leading VSYNC edge in the VSYNC and DOTCLK modes, or the beginning of every field in DVI mode
...
namespace CTRL2 {
constexpr LCDIF_Reg<&LCDIF_Layout::CTRL2, 3, 21> OUTSTANDING_REQS;   // This bitfield indicates the maximum number of outstanding transactions that LCDIF should request when it is acting as a bus master
    // 0b000..REQ_1
    // 0b001..REQ_2
    // 0b010..REQ_4
    // 0b011..REQ_8
    // 0b100..REQ_16
constexpr LCDIF_Reg<&LCDIF_Layout::CTRL2, 1, 20> BURST_LEN_8;        // By default, when the LCDIF is in the bus master mode, it will issue AXI bursts of length 16 (except when in packed 24 bpp mode, it will issue bursts of length 15)
...
// LCDIF General Control2 Register
namespace CTRL2_SET {
constexpr LCDIF_Reg<&LCDIF_Layout::CTRL2_SET, 3, 21, 0x0> OUTSTANDING_REQS;   // This bitfield indicates the maximum number of outstanding transactions that LCDIF should request when it is acting as a bus master
    // 0b000..REQ_1
    // 0b001..REQ_2
    // 0b010..REQ_4
    // 0b011..REQ_8
    // 0b100..REQ_16
constexpr LCDIF_Reg<&LCDIF_Layout::CTRL2_SET, 1, 20, 0x0> BURST_LEN_8;        // By default, when the LCDIF is in the bus master mode, it will issue AXI bursts of length 16 (except when in packed 24 bpp mode, it will issue bursts of length 15)
...
// LCDIF General Control2 Register
namespace CTRL2_CLR {
constexpr LCDIF_Reg<&LCDIF_Layout::CTRL2_CLR, 3, 21, 0x0> OUTSTANDING_REQS;   // This bitfield indicates the maximum number of outstanding transactions that LCDIF should request when it is acting as a bus master
    // 0b000..REQ_1
    // 0b001..REQ_2
    // 0b010..REQ_4
    // 0b011..REQ_8
    // 0b100..REQ_16
constexpr LCDIF_Reg<&LCDIF_Layout::CTRL2_CLR, 1, 20, 0x0> BURST_LEN_8;        // By default, when the LCDIF is in the bus master mode, it will issue AXI bursts of length 16 (except when in packed 24 bpp mode, it will issue bursts of length 15)
...
// LCDIF General Control2 Register
namespace CTRL2_TOG {
constexpr LCDIF_Reg<&LCDIF_Layout::CTRL2_TOG, 3, 21, 0x0> OUTSTANDING_REQS;   // This bitfield indicates the maximum number of outstanding transactions that LCDIF should request when it is acting as a bus master
    // 0b000..REQ_1
    // 0b001..REQ_2
    // 0b010..REQ_4
    // 0b011..REQ_8
    // 0b100..REQ_16
constexpr LCDIF_Reg<&LCDIF_Layout::CTRL2_TOG, 1, 20, 0x0> BURST_LEN_8;        // By default, when the LCDIF is in the bus master mode, it will issue AXI bursts of length 16 (except when in packed 24 bpp mode, it will issue bursts of length 15)
...
namespace VDCTRL0 {
constexpr LCDIF_Reg<&LCDIF_Layout::VDCTRL0,  1, 28> ENABLE_PRESENT;          // Setting this bit to 1 will make the hardware generate the ENABLE signal in the DOTCLK mode, thereby making it the true RGB interface along with the remaining three signals VSYNC, HSYNC and DOTCLK
constexpr LCDIF_Reg<&LCDIF_Layout::VDCTRL0,  1, 27> VSYNC_POL;               // Default 0 active low during VSYNC_PULSE_WIDTH time and will be high during the rest of the VSYNC period
constexpr LCDIF_Reg<&LCDIF_Layout::VDCTRL0,  1, 26> HSYNC_POL;               // Default 0 active low during HSYNC_PULSE_WIDTH time and will be high during the rest of the HSYNC period
constexpr LCDIF_Reg<&LCDIF_Layout::VDCTRL0,  1, 25> DOTCLK_POL;              // Default is data launched at negative edge of DOTCLK and captured at positive edge
constexpr LCDIF_Reg<&LCDIF_Layout::VDCTRL0,  1, 24> ENABLE_POL;              // Default 0 active low during valid data transfer on each horizontal line.
constexpr LCDIF_Reg<&LCDIF_Layout::VDCTRL0,  1, 21> VSYNC_PERIOD_UNIT;       // Default 0 for counting VSYNC_PERIOD in terms of DISPLAY CLOCK (pix_clk) cycles
constexpr LCDIF_Reg<&LCDIF_Layout::VDCTRL0,  1, 20> VSYNC_PULSE_WIDTH_UNIT;  // Default 0 for counting VSYNC_PULSE_WIDTH in terms of DISPLAY CLOCK (pix_clk) cycles
constexpr LCDIF_Reg<&LCDIF_Layout::VDCTRL0,  1, 19> HALF_LINE;               // Setting this bit to 1 will make the total VSYNC period equal to the VSYNC_PERIOD field plus half the HORIZONTAL_PERIOD field (i
constexpr LCDIF_Reg<&LCDIF_Layout::VDCTRL0,  1, 18> HALF_LINE_MODE;          // When this bit is 0, the first field (VSYNC period) will end in half a horizontal line and the second field will begin with half a horizontal line
...
constexpr LCDIF_Reg<&LCDIF_Layout::VDCTRL0_SET,  1, 28, 0x0> ENABLE_PRESENT;          // Setting this bit to 1 will make the hardware generate the ENABLE signal in the DOTCLK mode, thereby making it the true RGB interface along with the remaining three signals VSYNC, HSYNC and DOTCLK
constexpr LCDIF_Reg<&LCDIF_Layout::VDCTRL0_SET,  1, 27, 0x0> VSYNC_POL;               // Default 0 active low during VSYNC_PULSE_WIDTH time and will be high during the rest of the VSYNC period
constexpr LCDIF_Reg<&LCDIF_Layout::VDCTRL0_SET,  1, 26, 0x0> HSYNC_POL;               // Default 0 active low during HSYNC_PULSE_WIDTH time and will be high during the rest of the HSYNC period
constexpr LCDIF_Reg<&LCDIF_Layout::VDCTRL0_SET,  1, 25, 0x0> DOTCLK_POL;              // Default is data launched at negative edge of DOTCLK and captured at positive edge
constexpr LCDIF_Reg<&LCDIF_Layout::VDCTRL0_SET,  1, 24, 0x0> ENABLE_POL;              // Default 0 active low during valid data transfer on each horizontal line.
constexpr LCDIF_Reg<&LCDIF_Layout::VDCTRL0_SET,  1, 21, 0x0> VSYNC_PERIOD_UNIT;       // Default 0 for counting VSYNC_PERIOD in terms of DISPLAY CLOCK (pix_clk) cycles
constexpr LCDIF_Reg<&LCDIF_Layout::VDCTRL0_SET,  1, 20, 0x0> VSYNC_PULSE_WIDTH_UNIT;  // Default 0 for counting VSYNC_PULSE_WIDTH in terms of DISPLAY CLOCK (pix_clk) cycles
constexpr LCDIF_Reg<&LCDIF_Layout::VDCTRL0_SET,  1, 19, 0x0> HALF_LINE;               // Setting this bit to 1 will make the total VSYNC period equal to the VSYNC_PERIOD field plus half the HORIZONTAL_PERIOD field (i
constexpr LCDIF_Reg<&LCDIF_Layout::VDCTRL0_SET,  1, 18, 0x0> HALF_LINE_MODE;          // When this bit is 0, the first field (VSYNC period) will end in half a horizontal line and the second field will begin with half a horizontal line
...
constexpr LCDIF_Reg<&LCDIF_Layout::VDCTRL0_CLR,  1, 28, 0x0> ENABLE_PRESENT;          // Setting this bit to 1 will make the hardware generate the ENABLE signal in the DOTCLK mode, thereby making it the true RGB interface along with the remaining three signals VSYNC, HSYNC and DOTCLK
constexpr LCDIF_Reg<&LCDIF_Layout::VDCTRL0_CLR,  1, 27, 0x0> VSYNC_POL;               // Default 0 active low during VSYNC_PULSE_WIDTH time and will be high during the rest of the VSYNC period
constexpr LCDIF_Reg<&LCDIF_Layout::VDCTRL0_CLR,  1, 26, 0x0> HSYNC_POL;               // Default 0 active low during HSYNC_PULSE_WIDTH time and will be high during the rest of the HSYNC period
constexpr LCDIF_Reg<&LCDIF_Layout::VDCTRL0_CLR,  1, 25, 0x0> DOTCLK_POL;              // Default is data launched at negative edge of DOTCLK and captured at positive edge
constexpr LCDIF_Reg<&LCDIF_Layout::VDCTRL0_CLR,  1, 24, 0x0> ENABLE_POL;              // Default 0 active low during valid data transfer on each horizontal line.
constexpr LCDIF_Reg<&LCDIF_Layout::VDCTRL0_CLR,  1, 21, 0x0> VSYNC_PERIOD_UNIT;       // Default 0 for counting VSYNC_PERIOD in terms of DISPLAY CLOCK (pix_clk) cycles
constexpr LCDIF_Reg<&LCDIF_Layout::VDCTRL0_CLR,  1, 20, 0x0> VSYNC_PULSE_WIDTH_UNIT;  // Default 0 for counting VSYNC_PULSE_WIDTH in terms of DISPLAY CLOCK (pix_clk) cycles
constexpr LCDIF_Reg<&LCDIF_Layout::VDCTRL0_CLR,  1, 19, 0x0> HALF_LINE;               // Setting this bit to 1 will make the total VSYNC period equal to the VSYNC_PERIOD field plus half the HORIZONTAL_PERIOD field (i
constexpr LCDIF_Reg<&LCDIF_Layout::VDCTRL0_CLR,  1, 18, 0x0> HALF_LINE_MODE;          // When this bit is 0, the first field (VSYNC period) will end in half a horizontal line and the second field will begin with half a horizontal line
...
constexpr LCDIF_Reg<&LCDIF_Layout::VDCTRL0_TOG,  1, 28, 0x0> ENABLE_PRESENT;          // Setting this bit to 1 will make the hardware generate the ENABLE signal in the DOTCLK mode, thereby making it the true RGB interface along with the remaining three signals VSYNC, HSYNC and DOTCLK
constexpr LCDIF_Reg<&LCDIF_Layout::VDCTRL0_TOG,  1, 27, 0x0> VSYNC_POL;               // Default 0 active low during VSYNC_PULSE_WIDTH time and will be high during the rest of the VSYNC period
constexpr LCDIF_Reg<&LCDIF_Layout::VDCTRL0_TOG,  1, 26, 0x0> HSYNC_POL;               // Default 0 active low during HSYNC_PULSE_WIDTH time and will be high during the rest of the HSYNC period
constexpr LCDIF_Reg<&LCDIF_Layout::VDCTRL0_TOG,  1, 25, 0x0> DOTCLK_POL;              // Default is data launched at negative edge of DOTCLK and captured at positive edge
constexpr LCDIF_Reg<&LCDIF_Layout::VDCTRL0_TOG,  1, 24, 0x0> ENABLE_POL;              // Default 0 active low during valid data transfer on each horizontal line.
constexpr LCDIF_Reg<&LCDIF_Layout::VDCTRL0_TOG,  1, 21, 0x0> VSYNC_PERIOD_UNIT;       // Default 0 for counting VSYNC_PERIOD in terms of DISPLAY CLOCK (pix_clk) cycles
constexpr LCDIF_Reg<&LCDIF_Layout::VDCTRL0_TOG,  1, 20, 0x0> VSYNC_PULSE_WIDTH_UNIT;  // Default 0 for counting VSYNC_PULSE_WIDTH in terms of DISPLAY CLOCK (pix_clk) cycles
constexpr LCDIF_Reg<&LCDIF_Layout::VDCTRL0_TOG,  1, 19, 0x0> HALF_LINE;               // Setting this bit to 1 will make the total VSYNC period equal to the VSYNC_PERIOD field plus half the HORIZONTAL_PERIOD field (i
constexpr LCDIF_Reg<&LCDIF_Layout::VDCTRL0_TOG,  1, 18, 0x0> HALF_LINE_MODE;          // When this bit is 0, the first field (VSYNC period) will end in half a horizontal line and the second field will begin with half a horizontal line
...
namespace VDCTRL3 {
constexpr LCDIF_Reg<&LCDIF_Layout::VDCTRL3,  1, 29> MUX_SYNC_SIGNALS;     // When this bit is set, the LCDIF block will internally mux HSYNC with LCD_D14, DOTCLK with LCD_D13 and ENABLE with LCD_D12, otherwise these signals will go out on separate pins
constexpr LCDIF_Reg<&LCDIF_Layout::VDCTRL3,  1, 28> VSYNC_ONLY;           // This bit must be set to 1 in the VSYNC mode of operation, and 0 in the DOTCLK mode of operation.
constexpr LCDIF_Reg<&LCDIF_Layout::VDCTRL3, 12, 16> HORIZONTAL_WAIT_CNT;  // In the DOTCLK mode, wait for this number of clocks from falling edge (or rising if HSYNC_POL is 1) of HSYNC signal to account for horizontal back porch plus the number of DOTCLKs before the moving picture information begins
constexpr LCDIF_Reg<&LCDIF_Layout::VDCTRL3, 16,  0> VERTICAL_WAIT_CNT;    // In the VSYNC interface mode, wait for this number of DISPLAY CLOCK (pix_clk) cycles from the falling VSYNC edge (or rising if VSYNC_POL is 1) before starting LCD transactions and is applicable only if WAIT_FOR_VSYNC_EDGE is set
}  // namespace VDCTRL3

// LCDIF VSYNC Mode and Dotclk Mode Control Register4
namespace VDCTRL4 {
constexpr LCDIF_Reg<&LCDIF_Layout::VDCTRL4,  3, 29> DOTCLK_DLY_SEL;           // This bitfield selects the amount of time by which the DOTCLK signal should be delayed before coming out of the LCD_DOTCK pin
constexpr LCDIF_Reg<&LCDIF_Layout::VDCTRL4,  1, 18> SYNC_SIGNALS_ON;          // Set this field to 1 if the LCD controller requires that the VSYNC or VSYNC/HSYNC/DOTCLK control signals should be active at least one frame before the data transfers actually start and remain active at least one frame after the data transfers end
...
namespace LUT0_DATA {
constexpr LCDIF_Reg<&LCDIF_Layout::LUT0_DATA, 32, 0> DATA;  // Writing this field will load 4 bytes, aligned to four byte boundaries, of data indexed by the ADDR field of the REG_LUT_CTRL register
}  // namespace LUT0_DATA
...
namespace LUT1_DATA {
constexpr LCDIF_Reg<&LCDIF_Layout::LUT1_DATA, 32, 0> DATA;  // Writing this field will load 4 bytes, aligned to four byte boundaries, of data indexed by the ADDR field of the REG_LUT_CTRL register
}  // namespace LUT1_DATA
...
namespace PIGEON_0 {
template <size_t Index>
constexpr PIGEON_Reg<Index, &LCDIF_Layout::PIGEON_Layout::PIGEON_0,  8, 24> STATE_MASK;    // state_mask = (FS|FB|FD|FE) and (LS|LB|LD|LE) , select any combination of scan states as reference point for local counter to start ticking
    ...
    // 0b10000000..LINE END
template <size_t Index>
constexpr PIGEON_Reg<Index, &LCDIF_Layout::PIGEON_Layout::PIGEON_0, 12, 12> MASK_CNT;      // When the global counter selected through MASK_CNT_SEL matches value in this reg, pigeon local counter start ticking
template <size_t Index>
